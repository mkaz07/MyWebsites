...
class WebsiteTableViewController: UITableViewController,UISearchResultsUpdating, UISearchBarDelegate, UISearchControllerDelegate {
    //**Begin Copy**
    //2) Add filter search vars
    
    var filteredTableData = [NSManagedObject]()
    var resultSearchController = UISearchController()
    var websiteArray = [NSManagedObject]()
    ...
    func updateSearchResults(for searchController: UISearchController)
    {
        ...
        let searchPredicate = NSPredicate(format: "webName CONTAINS[c] %@", searchController.searchBar.text!)
        let array = (websiteArray as NSArray).filtered(using: searchPredicate)
    ...
    func loaddb()
    {
        ...
        let fetchRequest:NSFetchRequest<NSFetchRequestResult> = NSFetchRequest(entityName: "Website")
        
        do {
            let fetchedResults = try managedContext.fetch(fetchRequest) as? [NSManagedObject]
            if let results = fetchedResults {
                websiteArray = results
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (self.resultSearchController.isActive) {
            ...
            cell?.textLabel?.text = person.value(forKey: "webName") as! String?
            ...
            let cell =
                tableView.dequeueReusableCell(withIdentifier: "Cell")
                    as UITableViewCell?
            let person = websiteArray[(indexPath as NSIndexPath).row]
            cell?.textLabel?.text = person.value(forKey: "webName") as! String?
            cell?.detailTextLabel?.text = ">>"
            return cell!
        }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if editingStyle == .delete {
            ...
            if (self.resultSearchController.isActive) {
                ...
                context.delete(websiteArray[(indexPath as NSIndexPath).row])
            }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "UpdateWebsites" {
            if let destination = segue.destination as?
                ...
                if (self.resultSearchController.isActive) {
                    if let SelectIndex = (tableView.indexPathForSelectedRow as NSIndexPath?)?.row {
                        ...
                        destination.websitedb = selectedDevice
                        resultSearchController.isActive = false
                    }
                }
                else {
                    if let SelectIndex = (tableView.indexPathForSelectedRow as NSIndexPath?)?.row {
                        let selectedDevice:NSManagedObject = websiteArray[SelectIndex] as NSManagedObject
                        destination.websitedb = selectedDevice
                    }
                }
            }
